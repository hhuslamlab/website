# SLAM LAB Tutorials

## Programming

- [A simple guide for **Python** beginners](python.md)

- [A beginners guide to **debugging Python** programs](how_to_debug_101.md)

- [**Java** for the afraid (Under construction)](java.md)

- [Data serialization using **YAML**](yaml_intro.md)


## Tools

- [Ultimate Guide to **WSL2**](wsl.md)

- [Everything you need to know about **HPC**](hpc_intro.md)

- [**NVIDIA Jetson** tutorials](nvidia_setup.md)

- [Slamlab **website architecture**](website_setup.md)

- [A Guide to push **Github** around (under construction)](github_tut.md)

- [Installing **Aeneas** on WSL and MacOS](aeneas.md)

- [Installing **Kaldi** and **MFA** on Linux, WSL and MacOS](kaldimfa.md)

- [Write your paper in 1 second with these fabulous **Latex** tips - LaTeX for Linguists](latex_guide.md)

- [A beginner's guide to **Festival TTS**](festival_tts.md)

- [**Espeak-ng** for windows users](espeakng_windows.md)

- [A starter guide to **Docker** engine](docker.md)
